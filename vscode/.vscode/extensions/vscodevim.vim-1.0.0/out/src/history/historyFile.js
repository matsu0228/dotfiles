...
class HistoryFile {
    ...
    clear() {
        try {
            ...
            logger_1.logger.warn(`historyFile: Unable to delete ${this._historyFilePath}. err=${err}.`);
        }
    }
    save() {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                if (!(yield util.promisify(fs.exists)(this._historyDir))) {
                    yield util.promisify(mkdirp)(this._historyDir, 0o775);
                }
            }
            catch (err) {
                logger_1.logger.error(`historyFile: Failed to create directory. path=${this._historyDir}. err=${err}.`);
                throw err;
            }
            try {
                yield util.promisify(fs.writeFile)(this._historyFilePath, JSON.stringify(this._history), 'utf-8');
            }
            catch (err) {
                logger_1.logger.error(`historyFile: Failed to save history. path=${this._historyDir}. err=${err}.`);
                throw err;
            }
        });
    }
    load() {
        return __awaiter(this, void 0, void 0, function* () {
            let data = '';
            try {
                data = yield util.promisify(fs.readFile)(this._historyFilePath, 'utf-8');
            }
            catch (err) {
                if (err.code === 'ENOENT') {
