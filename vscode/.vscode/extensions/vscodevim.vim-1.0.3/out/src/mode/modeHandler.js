            ...
            if (!vimState.editor.document.isClosed && vimState.editor === vscode.window.activeTextEditor) {
                for (const { stop, i } of range_1.Range.IterateRanges(vimState.allCursors)) {
                    if (stop.line >= textEditor_1.TextEditor.getLineCount()) {
                        vimState.allCursors[i] = vimState.allCursors[i].withNewStop(vimState.cursorPosition.getDocumentEnd(vimState.editor));
                    }
    ...
    executeOperator(vimState) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!recordedState.operator) {
                logger_1.logger.warn('recordedState.operator: ' + recordedState.operator);
            ...
            const doTextEditorEdit = (command, edit) => {
                switch (command.type) {
                    ...
                    case 'moveCursor':
                        break;
                    default:
                        logger_1.logger.warn(`modeHandler: Unhandled text transformation type: ${command.type}.`);
                        break;
                }
