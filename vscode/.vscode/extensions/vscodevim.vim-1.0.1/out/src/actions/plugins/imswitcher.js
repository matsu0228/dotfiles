...
class InputMethodSwitcher {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if ((yield util_1.promisify(fs_1.exists)(rawObtainIMCmd)) || globals_1.Globals.isTesting) {
                try {
                    ...
                    logger_1.logger.error(`IMSwitcher: Error switching to default IM. err=${e}`);
                }
            }
            else {
                logger_1.logger.error(`IMSwitcher: Unable to find ${rawObtainIMCmd}. Check your 'vim.autoSwitchInputMethod.obtainIMCmd' in VSCode setting.`);
                this.disableIMSwitch();
            }
            ...
            if ((yield util_1.promisify(fs_1.exists)(rawSwitchIMCmd)) || globals_1.Globals.isTesting) {
                if (imKey !== '' && imKey !== undefined) {
                    ...
                    try {
                        ...
                        logger_1.logger.error(`IMSwitcher: Error switching to IM. err=${e}`);
                    }
                }
            }
            else {
                logger_1.logger.error(`IMSwitcher: Unable to find ${rawSwitchIMCmd}. Check your 'vim.autoSwitchInputMethod.switchIMCmd' in VSCode setting.`);
                this.disableIMSwitch();
            }
        });
    }
    ...
    disableIMSwitch() {
        logger_1.logger.warn('IMSwitcher: disabling IMSwitch');
        configuration_1.configuration.autoSwitchInputMethod.enable = false;
    }
    isConfigurationValid() {
        let switchIMCmd = configuration_1.configuration.autoSwitchInputMethod.switchIMCmd;
        if (!switchIMCmd.includes('{im}')) {
            logger_1.logger.error('IMSwitcher: vim.autoSwitchInputMethod.switchIMCmd is incorrect, \
        ...
        if (obtainIMCmd === undefined || obtainIMCmd === '') {
            logger_1.logger.error('IMSwitcher: vim.autoSwitchInputMethod.obtainIMCmd is empty');
            return false;
        }
        let defaultIMKey = configuration_1.configuration.autoSwitchInputMethod.defaultIM;
        if (defaultIMKey === undefined || defaultIMKey === '') {
            logger_1.logger.error('IMSwitcher: vim.autoSwitchInputMethod.defaultIM is empty');
            return false;
        }
        return true;
    }
