...
class Remapper {
    ...
    sendKey(keys, modeHandler, vimState) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            logger_1.logger.debug(`Remapper: find matching remap. keys=${keys}. mode=${mode_1.ModeName[vimState.currentMode]}.`);
            let remapping = Remapper.findMatchingRemap(userDefinedRemappings, keys, vimState.currentMode);
            if (remapping) {
        ...
        for (let remapping of configuration_1.configuration[this._configKey]) {
            ...
            if (remapping.commands) {
                for (const command of remapping.commands) {
                    ...
                    if (!configurationValidator_1.configurationValidator.isCommandValid(cmd)) {
            ...
            if (!remapping.after && !remapping.commands) {
                logger_1.logger.error(`Remapper: ${this._configKey}.
          Invalid configuration. Missing 'after' key or 'command'. ${debugMsg}`);
            ...
            let log = (msg) => (isCommandValid ? logger_1.logger.debug(msg) : logger_1.logger.warn(msg));
            log(`Remapper: ${this._configKey}. ${debugMsg}`);
            remappings[keys] = remapping;
        }
        return remappings;
    }
    static findMatchingRemap(userDefinedRemappings, inputtedKeys, currentMode) {
        let remapping;
