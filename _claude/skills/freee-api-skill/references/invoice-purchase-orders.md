...
| purchase_order_number | query | いいえ | string | 発注書番号 |
...
| start_purchase_order_date | query | いいえ | string(date) | 発注日の開始日 |
| end_purchase_order_date | query | いいえ | string(date) | 発注日の終了日 |
| start_collects_on | query | いいえ | string(date) | 支払予定日の開始日 |
| end_collects_on | query | いいえ | string(date) | 支払予定日の終了日 |
| limit | query | いいえ | integer | 取得レコードの件数 (デフォルト: 20, 最小: 1, 最大: 100)。limit と offset の合計は 10,000 を超えることはできません。 |
| offset | query | いいえ | integer | 取得レコードのオフセット (デフォルト: 0)。limit と offset の合計は 10,000 を超えることはできません。 |

### レスポンス (200)

The request has succeeded.

- purchase_orders (必須): array[object]
  配列の要素:
    - id (必須): integer(int64) - 発注書ID (最小: 1, 最大: 9223372036854775000)
    - company_id (必須): integer(int64) - 事業所ID (最小: 1, 最大: 9223372036854775000)
    - purchase_order_number (必須): string - 発注書番号
    - subject (必須): string - 件名
    - template_id (任意): integer(int64) - 帳票テンプレートID (最小: 1, 最大: 9223372036854775000)
    - purchase_order_date (必須): string(date) - 発注日 (パターン: ^[0-9]{4}-[0-9]{2}-[0-9]{2}$)
    - issue_date (任意): string(date) - 発生日 (パターン: ^[0-9]{4}-[0-9]{2}-[0-9]{2}$)
    - collects_on (任意): string(date) - 支払予定日 (パターン: ^[0-9]{4}-[0-9]{2}-[0-9]{2}$)
    - memo (必須): string - 社内メモ
...
説明: 発注書の作成をします。 issue_date, account_item_id, tax_code, item_id, section_id, tag_ids, segment_1_tag_id, segment_2_tag_id, segment_3_tag_id は、取引登録の下書き保存で利用されます。 tag_idsは10個まで設定可能です。
...
- purchase_order_number (任意): string - 発注書番号<br>
...
### GET /purchase_orders/templates

操作: 使用可能な発注書の帳票テンプレート一覧の取得


説明: 使用可能な発注書の帳票テンプレート一覧を返します。
...
説明: 発注書の更新をします。 issue_date, account_item_id, tax_code, item_id, section_id, tag_ids, segment_1_tag_id, segment_2_tag_id, segment_3_tag_id は、取引登録の下書き保存で利用されます。 tag_idsは10個まで設定可能です。
